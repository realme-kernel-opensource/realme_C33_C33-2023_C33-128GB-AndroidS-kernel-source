...
int ili_sleep_handler(int mode)
{
	...
	switch (mode) {
	case TP_SUSPEND:
		ILI_INFO("tp_suspend TP suspend start\n");
		...
		ILI_INFO("tp_suspend TP suspend end\n");
		break;
	case TP_DEEP_SLEEP:
		ILI_INFO("tp_suspend TP deep suspend start\n");
		ilits->tp_suspend = true;
		ilits->power_status = false;
		if (sense_stop) {
			if (ili_ic_func_ctrl("sense", DISABLE) < 0)
		...
		ILI_INFO("tp_suspend TP deep suspend end\n");
		break;
	case TP_RESUME:
		ILI_INFO("tp_resume start\n");
#if !RESUME_BY_DDI
