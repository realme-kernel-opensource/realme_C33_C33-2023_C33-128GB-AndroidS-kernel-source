...
static int fpsensor_init_device_power(fpsensor_data_t *fpsensor)
{
	int err = 0;
    struct device *dev = &fpsensor->platform_device->dev;

	g_fpsensor->reg_vdd = regulator_get(dev, "vddldo");
	if (IS_ERR(g_fpsensor->reg_vdd)) {
		err = PTR_ERR(g_fpsensor->reg_vdd);
        fpsensor_error("Regulator get failed vdd err=%d\n", err);
		return err;
	}

	err = regulator_set_voltage(g_fpsensor->reg_vdd, 3300000, 3300000);
	if (err) {
        fpsensor_error("regulator_set_voltage(%d) failed!\n", err);
		goto reg_vdd_put;
	}
    regulator_enable(g_fpsensor->reg_vdd);
    g_fpsensor->vdd_flag = 1;
	return err;
reg_vdd_put:
	regulator_put(g_fpsensor->reg_vdd);
	return err;
}
...
static int fpsensor_get_gpio_dts_info(fpsensor_data_t *fpsensor)
{
    ...
    ret = fpsensor_init_device_power(fpsensor);
    if (ret) {
        fpsensor_error("Failed to init_device_power. ret = %d\n", ret);
        return -1;
    }
...
static void fpsensor_hw_power_enable(u8 onoff)
{
    ...
    if (g_fpsensor->reg_vdd == NULL){
        fpsensor_debug("fpsensor_dev->reg_vdd is null! \n");
        return;
    }
    if (1 == onoff && 0 == g_fpsensor->vdd_flag){
        fpsensor_debug(" start power on\n");
        regulator_enable(g_fpsensor->reg_vdd);
        g_fpsensor->vdd_flag = 1;
    }else if (0 == onoff && 1 == g_fpsensor->vdd_flag){
        fpsensor_debug(" start power off\n");
        regulator_disable(g_fpsensor->reg_vdd);
        g_fpsensor->vdd_flag = 0;
    }
    fpsensor_debug("exit\n");
}
...
static void fpsensor_gpio_free(fpsensor_data_t *fpsensor)
{
    ...
    if (g_fpsensor->reset_gpio != 0) {
        ...
        g_fpsensor->reset_gpio = 0;
    }
    if (g_fpsensor->reg_vdd) {
        fpsensor_hw_power_enable(0);
        regulator_put(g_fpsensor->reg_vdd);
        g_fpsensor->reg_vdd = NULL;
    }
}
...
static irqreturn_t fpsensor_irq(int irq, void *handle)
{
    ...
    ** since this is interrupt context (other thread...) */
    smp_rmb();
    wake_lock_timeout(fpsensor_dev->ws, msecs_to_jiffies(1000));
    setRcvIRQ(1);
...
#endif
#endif
        wake_lock_destroy(fpsensor_dev->ws);
...
static int fpsensor_probe(struct platform_device *pdev)
{
    ...
    if ( FP_CHIPONE_C7319 != get_fpsensor_type()) {
        fpsensor_error("not found the chipone FP device.\n");
		return -ENODEV;
    }
    fpsensor_info("fp_vendor == FP_CHIPONE_C7319\n");
    ...
    fpsensor_dev->input             = NULL;
    fpsensor_dev->fb_status = 1;
    fpsensor_dev->reg_vdd           = NULL;
    fpsensor_dev->ws                = NULL;
    fpsensor_dev->vdd_flag          = 0;
	...
	fpsensor_dev->ws = wakeup_source_register(NULL, "fpsensor_ws");
	if (!fpsensor_dev->ws) {
        status = -ENOMEM;
        fpsensor_error("Failed to wake_lock_init fpsensor_ws.\n");
        goto release_drv_data;
	}
    fpsensor_dev->device_available = 1;
    //fpsensor_hw_power_enable(1);
...
{
    int status=0;
    fpsensor_info("fpsensor_init start.\n");
...
#endif
    if (status < 0) {
        fpsensor_error("Failed to register fpsensor driver.\n");
    }
    return status;
}
