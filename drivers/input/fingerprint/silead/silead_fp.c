...
static void silfp_get_screen_status(int status)
{
    ...
    if(fp_dev == NULL)
    {
        pr_err("kaoshan #### fp_dev is null return \n");
        return ;
    }

    pr_err("kaoshan #### %s %d   status = %d \n",__func__,__LINE__,status);
    if(status) {
        LOG_MSG_DEBUG(INFO_LOG, "[%s] LCD OFF\n", __func__);
        fp_dev->scr_off = 1;
...
silfp_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
    ...
    switch (cmd) {
    case SIFP_IOC_INIT:
         pr_err("kaoshan %s %d start ####",__func__,__LINE__);
        ...
        pr_err("kaoshan %s %d end ####",__func__,__LINE__);
        g_fp_dev = fp_dev;
...
static int silfp_probe(struct platform_device *spi)
{
    ...
    if (IS_ERR(silfp_class)) {
		pr_err("kaoshan #### %s %d  \n",__func__,__LINE__);
        unregister_chrdev(FP_DEV_MAJOR, "silead_fp");
...
int silfp_dev_init(void)
{
     ...
     */
	pr_err("silfp_dev_init start!\n");
    

    status = platform_driver_register(&silfp_driver);
    if (status < 0) {
        pr_err("[%s]  spi_register_driver fail ret=%d.\n", __func__, status);
        return status;
    }
	pr_err("silfp_dev_init finish!\n");
