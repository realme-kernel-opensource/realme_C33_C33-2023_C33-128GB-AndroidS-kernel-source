...
static int ff_ctl_enable_irq(bool on)
{
    ...
    FF_LOGD("irq: '%s',irq_flag is %d \n", on ? "enable" : "disabled",irq_flag);
    ...
    if ((on) && (!irq_flag)) {
		...
		FF_LOGD("no neeed enable or disable irq ,becasue on is %d,irq_flag is %d  \n",on,irq_flag);
	}
...
static void ff_ctl_device_event(struct work_struct *ws)
{
    ...
    FF_LOGI("%s(irq = %d, ..) toggled.", __func__, ctx->irq_num);
#ifdef CONFIG_PM_WAKELOCKS
    __pm_wakeup_event(g_context->wake_lock, jiffies_to_msecs(2*HZ));
#else
    wake_lock_timeout(&g_context->wake_lock, 10 * HZ); // 10 seconds.
#endif
...
#else
//add for sprd get suspend & resume
static void ft_get_screen_status(int status)
{
	char *uevent_env[2];
	FF_LOGV("'%s' enter,get status  is %d ...", __func__,status);
    if(g_context->miscdev.this_device == NULL)
    {
        FF_LOGE("'%s' leave.", __func__);
        return ;
    }
    if(status) {    
        //screen_flag = 1;
...
#else
	fingerprint_get_screen_status = ft_get_screen_status;
#endif
...
static long ff_ctl_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
    ...
    case FF_IOC_SYNC_CONFIG: {
        ...
        g_log_level = g_config_t->log_level;
        break;
    }
...
static int __init ff_ctl_driver_init(void)
{
    ...
    if ( FP_FOCALTECH_9396 != get_fpsensor_type()) {
        FF_LOGE("not found the focaltech FP device.\n");
        g_driver_init_status = false;
        return 0;
    }

	FF_LOGI("fp_vendor == FP_FOCALTECH_9396\n");
...
static void __exit ff_ctl_driver_exit(void)
{
    ...
    if (!g_driver_init_status) return;
...
MODULE_DESCRIPTION("The device control driver for FocalTech's fingerprint sensor.(with log opened");
