...
#undef pr_fmt
#endif
#define pr_fmt(fmt) "FLASH_sy7807_p50: %d %d %s : " fmt, current->pid, __LINE__, __func__

#define FLASH_IC_DRIVER_NAME       "sprd_sy7807_p50"
#define FLASH_PWM_NAME             "flash_sy7807_p50"
#define TORCH_PWM_NAME             "torch_sy7807_p50"
...
static int sprd_flash_ic_open_preflash(void *drvd, uint8_t idx)
{
	...
	if (drv_data->torch_led_index < 2) {
		drv_data->torch_led_index = 2;
	} else if (drv_data->torch_led_index > 99) {
		drv_data->torch_led_index = 99;
	}

	if (SPRD_FLASH_LED0 & idx){
		pr_info("---Hui---open  preflash :pwm , torch_led_index:%d",drv_data->torch_led_index);
...
static int sprd_flash_ic_open_highlight(void *drvd, uint8_t idx)
{
	...
	if(drv_data->torch_led_index < 4){
		drv_data->torch_led_index = 4;
	}else if (drv_data->torch_led_index >68){
		drv_data->torch_led_index = 68;
	}
...
static const struct of_device_id sprd_flash_ic_of_match_table[] = {
	{.compatible = "sprd,pwm-sy7807_p50"},
};
...
static int sprd_flash_ic_driver_probe(struct platform_device *pdev)
{
	...
	pr_info("sy7807_p50-sprd_flash_ic_driver_probe     E");
	if (IS_ERR(pdev))
		return -EINVAL;

	//pr_err("---LYS--sy7807_p50-sprd_flash_ic_driver_probe     E");
	drv_data = devm_kzalloc(&pdev->dev, sizeof(*drv_data), GFP_KERNEL);
	if (!drv_data)
		return -ENOMEM;

	platform_set_drvdata(pdev, drv_data);
	wakeup_source_init(&drv_data->ws,"Flash ic sy7807_p50 Wakeup Source");
...
exit:
	pr_err("sy7807_p50-sprd_flash_ic_driver_probe       X");
	return ret;
...
static int sprd_flash_ic_driver_remove(struct platform_device *pdev)
{
	...
	if (!drv_data)
		return -ENOMEM;
	wakeup_source_trash(&drv_data->ws);
...
MODULE_DESCRIPTION("Sprd sy7807_p50 Flash Driver");
MODULE_LICENSE("GPL");
