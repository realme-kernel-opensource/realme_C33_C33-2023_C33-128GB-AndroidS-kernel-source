        ...
        unsigned long action, void *data)
{
    ...
    if (evdata && evdata->data) {
        ...
        } else if (action == FB_BLANK_POWERDOWN) {  // FB_EARLY_EVENT_BLANK zx for to test
            blank = *(int *)evdata->data;
...
#ifdef CONFIG_T_PRODUCT_INFO
int chipone_get_tp_fw_info(char *buf, void *arg0) 
{
//	if(tp_id == 1){
		return sprintf(buf, "%s-%s-%s-v%04x", "hf","K630","icnl9911c",icnl_fireware_num);
//	}else if (tp_id == 2){
//		return sprintf(buf, "%s-%s-%s-v%04x", "ls",CONFIG_ARCH_MTK_PROJECT,"icnl9911c",icnl_fireware_num);
//	}else if (tp_id == 3){
//		return sprintf(buf, "%s-%s-%s-v%04x", "boe",CONFIG_ARCH_MTK_PROJECT,"icnl9911c",icnl_fireware_num);
//	}else{
//		return sprintf(buf, "%s-%s-%s-v%04x", "unknow",CONFIG_ARCH_MTK_PROJECT,"unknow",icnl_fireware_num);
//	}
}
#endif
...
#endif
{
...
#endif
    cts_info("cts_driver_probe start");
	...
	{
		int chipone_get_tp_fw_info(char *buf, void *arg0);
		FULL_PRODUCT_DEVICE_CB(ID_TP, chipone_get_tp_fw_info, NULL);
	}
	#endif
    return 0;
