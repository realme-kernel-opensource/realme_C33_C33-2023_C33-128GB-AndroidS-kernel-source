...
static int cts_plat_irq_thread_fn(void *arg)
{
    ...
    //struct sched_param sched_param = { .sched_priority = 4 }; // zx for to test

    cts_info("IRQ thread start ...");

    //sched_setscheduler(current, SCHED_RR, &sched_param); // zx for to test
    do {
...
int cts_plat_request_resource(struct cts_platform_data *pdata)
{
    ...
    ret = gpio_request(pdata->int_gpio, "chipone,irq-gpio");
    if (ret) {
    ...
    ret = gpio_request(pdata->rst_gpio, "chipone,rst-gpio");
    if (ret) {
