...
int32_t xrp_dump_libraries(struct xvp * xvp)
{
	...
	/*dump vdsp firmware */
	sprintf(filename, VDSP_DUMP_PATH "vdsp_firmware_%d.dump",
		xvp->sysdump_num);
#ifdef IOMMUANDMEM
	ret =
	    xrp_save_file(filename, xvp_buf_get_vaddr(xvp->fw_buf),
			  VDSP_FIRMWIRE_SIZE);
#else
	ret =
	    xrp_save_file(filename, xvp->firmware_viraddr, VDSP_FIRMWIRE_SIZE);
#endif
